...
function api_user_post($request){
  ...
  if(username_exists($username) || email_exists($email)){
    $response = new WP_Error('error','Email ou Login já cadastrado',['status' => 403]);
    return rest_ensure_response($response);
  }
...
function register_api_user_post(){
  register_rest_route('api','/user',['methods' => WP_REST_Server::CREATABLE,'callback' => 'api_user_post']);
}

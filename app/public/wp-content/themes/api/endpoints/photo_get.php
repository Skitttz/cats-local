...
function photo_data($post) {
  ...
  $src = isset($post_meta['img'][0]) ? wp_get_attachment_image_src($post_meta['img'][0], 'large')[0] : null;
  $user = get_userdata($post->post_author);
  $total_comments = get_comments_number($post->ID);
  $total_likes = get_total_likes($post->ID);


  $descricao = isset($post_meta['descricao'][0]) ? $post_meta['descricao'][0] : null;
  $idade = isset($post_meta['idade'][0]) ? $post_meta['idade'][0] : null;
  $acessos = isset($post_meta['acessos'][0]) ? $post_meta['acessos'][0] : null;
...
function register_api_photo_get(){
  register_rest_route('api','/photo/(?P<id>[0-9]+)',['methods' => WP_REST_Server::READABLE,'callback' => 'api_photo_get']);
}
...
function register_api_photos_get(){
  register_rest_route('api','/photo',['methods' => WP_REST_Server::READABLE,'callback' => 'api_photos_get']);
}
